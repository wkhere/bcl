...
func TestInterpretFromPy(t *testing.T) {
	var tab = []struct {
		...
		{`126.3`, `def x{}; def x{}; bind x:1   -> struct`, "", false, true, `found 2 blocks of type x`},
		{`126.4`, `def x{}; def x{}; bind x:all -> struct`, "", false, true, `bind of multiple blocks`},
		{`126.5`, `def x{}; bind x->slice; bind x->struct`, "", false, false, ""},
		{`126.6`, `def x{}; bind x: -> slice`, "", false, true, `expected block selector`},
		{`126.7`, `def x{}; bind x:foo -> slice`, "", false, true, `expected block selector`},
		{`127.1`, `def b "foo"{}; bind b:"foo" -> struct`, "== /dev/stdin ==\n0000   1:13  DEFBLOCK      0 'b'\t   1 'foo'\n0003   1:14  ENDBLOCK\n0004   1:38  BINDNB        0 'b'\t   2 'foo'\t0x14\n0008      |  RET", true, false, ""},
		{`127.2`, `def b "foo"{}; def b "bar"{}; bind b:"bar" -> struct`, "== /dev/stdin ==\n0000   1:13  DEFBLOCK      0 'b'\t   1 'foo'\n0003   1:14  ENDBLOCK\n0004   1:28  DEFBLOCK      0 'b'\t   2 'bar'\n0007   1:29  ENDBLOCK\n0008   1:53  BINDNB        0 'b'\t   3 'bar'\t0x14\n0012      |  RET", true, false, ""},
		{`128.1`, `bind x:"foo" -> struct`, "", false, true, `no blocks of type x`},
		{`128.2`, `def x{}; bind x:"foo" -> struct`, "", false, true, `block x:"foo" not found`},
		{`128.3`, `def x "a"{}; bind x:"foo" -> struct`, "", false, true, `block x:"foo" not found`},
		{`128.4`, `def x "a"{}; bind x:"foo" -> slice`, "", false, true, `block x:"foo" not found`},
		{`128.5`, `def x "a"{}; def x "b"{}; bind x:"a" -> struct`, "", false, false, ""},
		{`128.6`, `def x "a"{}; def x "b"{}; bind x:"b" -> struct`, "", false, false, ""},
		{`128.7`, `def x "a"{}; bind x:"a" -> slice`, "", false, false, ""},
		{`128.8`, `def x "a"{}; def x "b"{}; bind x:"a" -> slice`, "", false, false, ""},
		{`128.9`, `def x "a"{}; def x "b"{}; bind x:"b" -> slice`, "", false, false, ""},
		{`129.1`, `def b "foo"{}; bind b:"foo", -> slice`, "== /dev/stdin ==\n0000   1:13  DEFBLOCK      0 'b'\t   1 'foo'\n0003   1:14  ENDBLOCK\n0004   1:38  BINDNBS       0 'b'\t   2 'foo'\t0x25\n0009      |  RET", true, false, ""},
		{`129.2`, `def b "foo"{}; def b "bar"{}; bind b:"foo","bar" -> slice`, "== /dev/stdin ==\n0000   1:13  DEFBLOCK      0 'b'\t   1 'foo'\n0003   1:14  ENDBLOCK\n0004   1:28  DEFBLOCK      0 'b'\t   2 'bar'\n0007   1:29  ENDBLOCK\n0008   1:58  BINDNBS       0 'b'\t   3 'foo'\t   4 'bar'\t0x25\n0014      |  RET", true, false, ""},
		{`130.1`, `def x "a"{}; bind x:"foo", -> struct`, "", false, true, `block x:"foo" not found`},
		{`130.2`, `def x "a"{}; bind x:"a", -> struct`, "", false, true, `invalid bind target and selector`},
		{`130.3`, `def x "a"{}; bind x:"a",   -> slice`, "", false, false, ""},
		{`130.4`, `def x "a"{}; def x "b"{}; bind x:"a","b"  -> slice`, "", false, false, ""},
		{`130.5`, `def x "a"{}; def x "b"{}; bind x:"a","b", -> slice`, "", false, false, ""},
		{`130.6`, `def x "a"{}; def x "b"{}; bind x:"c", -> slice`, "", false, true, `block x:"c" not found`},
		{`131.1`, `bind {}`, "== /dev/stdin ==\n0000    1:7  DEFUBIND\n0001    1:8  ENDUBIND\n0002      |  RET", true, false, ""},
		{`131.2`, `def x{}; bind {x -> struct}`, "== /dev/stdin ==\n0000    1:7  DEFBLOCK      0 'x'\t   1 ''\n0003    1:8  ENDBLOCK\n0004   1:16  DEFUBIND\n0005   1:27  BIND          0 'x'\t0x11\n0008   1:28  ENDUBIND\n0009      |  RET", true, false, ""},
		{`131.3`, `def x{}; def y{}; bind {x -> struct; y:all -> slice}`, "== /dev/stdin ==\n0000    1:7  DEFBLOCK      0 'x'\t   1 ''\n0003    1:8  ENDBLOCK\n0004   1:16  DEFBLOCK      2 'y'\t   1 ''\n0007   1:17  ENDBLOCK\n0008   1:25  DEFUBIND\n0009   1:36  BIND          0 'x'\t0x11\n0012   1:52  BIND          2 'y'\t0x2F\n0015   1:53  ENDUBIND\n0016      |  RET", true, false, ""},
		{`131.4`, `def x{}; def y{}; bind {y -> struct; x:all -> slice}`, "== /dev/stdin ==\n0000    1:7  DEFBLOCK      0 'x'\t   1 ''\n0003    1:8  ENDBLOCK\n0004   1:16  DEFBLOCK      2 'y'\t   1 ''\n0007   1:17  ENDBLOCK\n0008   1:25  DEFUBIND\n0009   1:36  BIND          2 'y'\t0x11\n0012   1:52  BIND          0 'x'\t0x2F\n0015   1:53  ENDUBIND\n0016      |  RET", true, false, ""},
		{`132.1`, `bind {}`, "", false, false, ""},
		{`132.2`, `def x{}; bind {x -> struct}`, "", false, false, ""},
		{`132.3`, `def x{}; bind {x:all -> slice}`, "", false, false, ""},
		{`132.4`, `def x{}; def y{}; bind {x -> struct; y -> struct}`, "", false, false, ""},
		{`132.5`, `def x{}; def y{}; bind {x -> struct; y:all -> slice}`, "", false, false, ""},
		{`132.6`, `def x{}; def y{}; bind {x:all -> slice; y -> struct}`, "", false, false, ""},
		{`133.1`, `bind {42x}`, "", false, true, `invalid syntax`},
		{`133.2`, `bind {x -> struct}`, "", false, true, `no blocks of type x`},
		{`133.3`, `def x{}; bind {x:"a" -> struct}`, "", false, true, `block x:"a" not found`},
		{`133.4`, `def x{}; bind {x:"a" -> struct}`, "", false, true, `block x:"a" not found`},
